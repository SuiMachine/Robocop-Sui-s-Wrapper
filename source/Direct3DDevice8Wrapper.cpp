...
HRESULT Direct3DDevice8Wrapper::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value) {
	...
	case(D3DRS_FOGEND):
	{
		float originalValue = *reinterpret_cast<float*>(&Value);
		*reinterpret_cast<float*>(&Value) = originalValue * FogEndMultiplier;
		printf("Fog end distance: %f -> %f\n", originalValue, originalValue * FogEndMultiplier);
	}	break;
	}
...
HRESULT Direct3DDevice8Wrapper::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value) {
	if (AnisotropicLevel > 0)
	{
		...
		ret = Direct3DDevice8->SetTextureStageState(Stage, D3DTSS_MAXANISOTROPY, AnisotropicLevel);
	}
